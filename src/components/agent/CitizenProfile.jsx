...
export default function CitizenProfile({ role = "agente" }) {
  ...
  // ✅ Recientes: ahora consulta la colección real "citas" (no collectionGroup)
  useEffect(() => {
    if (!dniNorm) {
      setRecientes([]);
      return;
  ...
  const buscar = async () => {
    ...
    try {
      ...
      // 2) Secundario: si es admin, intentar en usuarios
      if (isAdmin) {
        const qU = query(
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <p style={styles.sub}>
          Busca por DNI/RUT, edita o crea si no existe. La ficha se guarda en “ciudadanos”.
          <br />
        ...
        <input
          style={styles.input}
          value={dniRaw}
          onChange={(e) => setDniRaw(e.target.value)}
          placeholder="18.373.138-1"
          disabled={disabled}
        />
        <div style={{ ...styles.sub, marginTop: 8 }}>
          Normalizado: <b>{dniNorm || "-"}</b>
        </div>

        <div style={styles.label}>Tipo Documento</div>
        <select style={styles.select} value={tipoDoc} onChange={(e) => setTipoDoc(e.target.value)} disabled={disabled}>
        ...
        <input
          ...
          placeholder="Si se deja vacío, se arma con Nombre + Apellido"
        />

        <div style={styles.label}>Teléfono (opcional)</div>
        <input style={styles.input} value={telefono} onChange={(e) => setTelefono(e.target.value)} placeholder="+56 9 1234 5678" />

        <div style={styles.label}>Email (opcional)</div>
        <input style={styles.input} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="correo@dominio.com" />
      ...
      <div style={styles.card}>
        ...
        {recientes.length === 0 ? (
          <div style={styles.recItem}>Sin registros recientes.</div>
        ) : (
          recientes.map((r) => (
            <div key={r.id} style={styles.recItem}>
              <div style={{ fontWeight: 900 }}>
                {r.codigo || "-"} <span style={{ fontWeight: 800, color: "#666" }}>({r.estado || "-"})</span>
              </div>
              <div style={{ marginTop: 6, fontSize: 12, fontWeight: 800, color: "#333" }}>
